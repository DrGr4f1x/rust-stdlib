fn main() {
    ...
    // Access arbitrary elements without bounds checking
    let second = fruits[1];
    println!("Second fruit: {}", second);
    ...
    // Remove some item and shift all that come after it 
    // into place
    ...
    let mut fridge = vec!["Beer", "Leftovers", "Mayonnaise"];
    println!("Is the fridge empty? {}", fridge.is_empty());
    // Remove all elements
    fridge.clear();
    println!("Is the fridge now empty? {}", fridge.is_empty());

    // We can split a vector in two and combine them again
