fn main() {
    ...
    if let Some(letter) = letter {
        println!("The fourth letter of the alphabet is: {}", letter);
    }
    ...
    println!("The last 8 letters in a range from 0 to 10: {:?}", nums);

    // take and skip accept predicates in the form of
    // take_while and skip_while
    let nums: Vec<_> = (0..).take_while(|x| x * x < 50).collect();
    println!("All positive numbers whose squares are less than 50: {:?}", nums);
    ...
    println!(
        "When operating on the first 10 positive numbers\n\
        their sum is {} and\n\
        their product is {}.",
        sum, product);
    ...
    // Combine an iterator with itself, makeing it infinite.
    ...
    // Combine iterators by putting them one after the other
    ...
    // Because zip is lazy, you can use two infinite ranges
